...
const MealCard = ({ meal }: { meal: IMeal }) => {
  ...
  return (
    <Card
      key={meal?._id}
      className="relative p-6 px-4 shadow-lg rounded-xl transition-transform hover:scale-105 bg-white flex flex-col h-full"
    >
      ...
      <CardContent className="text-center space-y-4 flex flex-col flex-grow">
        ...
        <h3 className="text-xl font-semibold text-primary min-h-[48px] flex items-center justify-center">
          {meal?.name}
        </h3>

        {/* Cuisine Type and Dietary Preferences */}
        <div className="flex justify-center gap-2 mt-2 flex-wrap">
          <span className="bg-gray-200 text-xs px-3 py-1 rounded-full text-gray-800">
            {meal?.cuisineType}
          </span>
        ...
        <p className="text-gray-600 text-sm mt-2 min-h-[60px] line-clamp-3">
          {meal?.description}
        </p>
